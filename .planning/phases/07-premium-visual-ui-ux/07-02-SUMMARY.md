...
provides:
  ...
  - Official PUBG API weapon renders for 28 seed weapons plus authored JS9 fallback silhouette
...
tech-stack:
  ...
  patterns:
    ...
    - WeaponIcon exposes data-silhouette-id, data-support-kind, and data-art-source for visual contract tests
    - Official PUBG API assets are stored locally under public/pubg-api-assets and never hotlinked
...
key-decisions:
  ...
  - "QBZ and QBU expose explicit current-patch lifecycle labels from the patch catalog while still retaining visual renders."
  - "After user visual feedback, available official PUBG API weapon renders replaced most authored SVGs; JS9 remains authored because the upstream api-assets repository does not expose a JS9 file."
...
patterns-established:
  - "WeaponIcon resolves by seed id, seed name, and aliases through weaponVisualRegistry, prefers local PUBG API renders when available, and falls back to authored category/JS9 silhouettes when not."
...
# Phase 07 Plan 02: 29 Weapon Visual Catalog And Support Status Contract Summary

**29-weapon visual catalog with local PUBG API weapon renders where available, authored JS9 fallback, honest visual-vs-technical support status, and desktop/mobile screenshot evidence.**

## Performance

- **Duration:** 31 min including visual correction follow-up
- **Started:** 2026-05-07T04:10:26Z
- **Completed:** 2026-05-07T04:41:03Z
- **Tasks:** 3/3
- **Files modified/added in follow-up correction:** 38
...
- Refactored `WeaponIcon` to prefer local official PUBG API weapon renders for 28 current seed weapons, while keeping authored SVG fallback behavior for JS9 and future/non-seed weapons.
- Added a production-blocked visual matrix route and Playwright checks that capture all 29 weapons on desktop and mobile.
- Responded to user visual feedback by replacing the too-abstract authored silhouettes with recognizable in-game weapon renders from the official PUBG API developer asset repository.
...
4. **Follow-up: replace abstract SVGs with local PUBG API weapon renders after user visual feedback** - current correction commit

**Plan metadata:** completed in `1742735` and updated by the visual correction commit.
...
- `src/ui/components/weapon-icon.tsx` - Registry-backed renderer using local PUBG API weapon images where available and authored SVG fallbacks where needed.
- `src/ui/components/weapon-icon.module.css` - Stable frame, category, asset-image, and status-label styling.
...
- `public/pubg-api-assets/weapons/main/*.png` - Local copies of available official PUBG API weapon renders.
- `public/pubg-api-assets/README.md` - Source and usage notes for PUBG API assets.
...
- Used official PUBG API developer weapon renders from `pubg/api-assets` instead of unofficial wiki/site images or copied website media. These remain local, non-exclusive visual references, and do not change the monetization boundary.
...
- `npx vitest run src/ui/components/weapon-icon.contract.test.tsx src/ui/components/weapon-visual-registry.test.ts src/ui/components/weapon-support-status.test.ts src/app/dashboard/page.contract.test.ts src/app/history/page.contract.test.ts` - PASS, 5 files / 21 tests.
- `npx playwright test e2e/phase7.weapon-icons.spec.ts` - PASS, 2 tests; desktop and mobile screenshots generated.
- `npx vitest run` - PASS, 158 files / 865 tests.
...
Ready for `07-03`: analyze/upload/result surfaces can now consume a complete 29-weapon visual catalog without implying technical calibration for every seed weapon.

## Self-Check: PASSED

- Summary file created.
- All plan tasks have task commits.
- All 29 seed weapons resolve to non-generic visuals; 28 use local official PUBG API renders and JS9 uses an authored fallback because no upstream JS9 asset was available.
...
- Official PUBG API weapon renders are sourced from `pubg/api-assets`, stored locally, not hotlinked, and not used as a paid-exclusive data feature.
