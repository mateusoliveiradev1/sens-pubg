...
#### Scenario: Weekly challenge prefers an active mission when one exists
- **WHEN** the current week has an active weekly mission already curated for the season or ritual window
- **THEN** the weekly challenge board uses that mission as its primary source before falling back to trend-derived or editorial prompts
