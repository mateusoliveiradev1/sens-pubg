...
### Requirement: Explainable XP and level ladder
The system SHALL make XP gains and level progression legible enough that players understand why they advanced and what the next milestone is.

#### Scenario: User earns XP from an eligible action
- **WHEN** an authenticated user completes an eligible action that awards XP
- **THEN** the system can show a concise explanation of the XP delta, the current level state, and the next milestone without exposing private progression detail to other viewers

#### Scenario: User crosses a level threshold
- **WHEN** a newly applied progression event moves a user across a configured level boundary
- **THEN** the system records the level-up outcome and can surface the unlocked milestone, reward, or next target with factual language

### Requirement: Active mission board
The system SHALL provide logged-in users with a prioritized board of active missions instead of flooding the interface with every possible objective.

#### Scenario: User has active missions
- **WHEN** an authenticated user with active missions opens the community hub
- **THEN** the experience shows a small prioritized set of current objectives with progress, reward value, and the next meaningful action

#### Scenario: User has many possible mission candidates
- **WHEN** more active missions exist than the hub should show at once
- **THEN** the system prioritizes the most urgent or relevant missions and avoids rendering a long serial wall of mission cards
