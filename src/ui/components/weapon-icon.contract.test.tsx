...
describe('WeaponIcon premium catalog contract', () => {
    it('renders all 29 seed weapons with authored non-generic silhouettes and accessible labels', () => {
        for (const entry of weaponVisualRegistry) {
            ...
            expect(html, entry.displayName).toContain('data-art-source="authored-sens-pubg"');
    ...
    it('does not import official or extracted PUBG/KRAFTON image assets', () => {
        const source = readFileSync(new URL('./weapon-icon.tsx', import.meta.url), 'utf8');

        expect(source).not.toMatch(/<image|href=|png|jpg|webp|krafton|official|battlegrounds/i);
