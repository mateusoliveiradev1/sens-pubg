...
import { buildCapturedReviewDecisionTemplate } from '../src/core/captured-review-decisions-template';
...
async function tryReadJson<T>(filePath: string, parser: (value: unknown) => T): Promise<T | undefined> {
...
async function main(): Promise<void> {
    ...
    if (!datasetArg || !intakeArg || !labelsArg || !outputArg) {
        throw new Error('usage: tsx scripts/generate-captured-review-decisions.ts <dataset> <intake> <labels> <output> [createdAt]');
    }
    ...
    const existingDecisionSet = await tryReadJson(outputArg, parseCapturedBenchmarkReviewDecisionSet);
    const plan = buildCapturedBenchmarkPlan(dataset, intakeManifest, labelSet);
    const template = buildCapturedReviewDecisionTemplate({
        decisionSetId: `captured-review-decisions-todo-${new Date(createdAtArg ?? Date.now()).toISOString().slice(0, 10)}`,
        createdAt: createdAtArg ?? new Date().toISOString(),
        intakeManifest,
        labelSet,
        plan,
        ...(existingDecisionSet ? { existingDecisionSet } : {}),
    });
    const outputPath = resolveInputPath(outputArg);

    await writeFile(outputPath, `${JSON.stringify(template, null, 2)}\n`, 'utf8');
    console.log(`captured review decisions written to ${outputPath}`);
}
