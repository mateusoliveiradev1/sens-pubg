...
export const hasPhase7AuthSecret = Boolean(AUTH_SECRET);
...
export async function signInAsPhase7User(page: Page, user: { id: string; email: string; name: string }) {
...
export async function seedPhase7HistoryFixture() {
    ...
    const user = {
        id: userId,
        email: `phase7-final-${suffix}@example.com`,
        name: `Phase 7 Final ${suffix.slice(0, 8)}`,
    };
